...
class _netD_CIFAR10_SNGAN(nn.Module):
    def __init__(self, ngpu, num_classes=10):
        super(_netD_CIFAR10_SNGAN, self).__init__()
        self.ngpu = ngpu

        # Convolution 1
        self.conv1 = nn.Sequential(
            spectral_norm(nn.Conv2d(3, 64, 3, 1, 1, bias=False)),
        ...
        # Convolution 3
        self.conv3 = nn.Sequential(
            spectral_norm(nn.Conv2d(64, 128, 3, 1, 1, bias=False)),
            nn.LeakyReLU(0.1, inplace=True),
        )
        # Convolution 4
        self.conv4 = nn.Sequential(
            spectral_norm(nn.Conv2d(128, 128, 4, 2, 1, bias=False)),
            nn.LeakyReLU(0.1, inplace=True),
        )
        # Convolution 5
        self.conv5 = nn.Sequential(
            spectral_norm(nn.Conv2d(128, 256, 3, 1, 1, bias=False)),
            nn.LeakyReLU(0.1, inplace=True),
        )
        # Convolution 6
        self.conv6 = nn.Sequential(
            spectral_norm(nn.Conv2d(256, 256, 4, 2, 1, bias=False)),
            nn.LeakyReLU(0.1, inplace=True),
        )
        # Convolution 7
        self.conv7 = nn.Sequential(
            spectral_norm(nn.Conv2d(256, 512, 3, 1, 1, bias=False)),
            nn.LeakyReLU(0.1, inplace=True),
        )
        # discriminator fc
        self.fc_dis = spectral_norm(nn.Linear(4*4*512, 1))
        # aux-classifier fc
        self.fc_aux = spectral_norm(nn.Linear(4*4*512, num_classes))
        # softmax and sigmoid
        self.softmax = nn.Softmax()
        self.sigmoid = nn.Sigmoid()
    ...
    def forward(self, input):
        if isinstance(input.data, torch.cuda.FloatTensor) and self.ngpu > 1:
            ...
            conv5 = nn.parallel.data_parallel(self.conv5, conv4, range(self.ngpu))
            conv6 = nn.parallel.data_parallel(self.conv6, conv5, range(self.ngpu))
            conv7 = nn.parallel.data_parallel(self.conv7, conv6, range(self.ngpu))
            flat7 = conv7.view(-1, 4*4*512)
            fc_dis = nn.parallel.data_parallel(self.fc_dis, flat7, range(self.ngpu))
            fc_aux = nn.parallel.data_parallel(self.fc_aux, flat7, range(self.ngpu))
        else:
            conv1 = self.conv1(input)
            conv2 = self.conv2(conv1)
            conv3 = self.conv3(conv2)
...
class _netG_CIFAR10_SNGAN(nn.Module):
    def __init__(self, ngpu, nz):
        super(_netG_CIFAR10_SNGAN, self).__init__()
        self.ngpu = ngpu
        self.nz = nz
