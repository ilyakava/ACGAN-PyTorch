...
class ResBlockGenerator(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        if stride != 1:
            self.bypass = nn.Sequential(nn.Upsample(scale_factor=2) )
...
class Discriminator(nn.Module):
    def __init__(self, opt):
        ...
        self.feat_net = nn.Sequential(
            ...
            ResBlockDiscriminator(ndf, ndf, stride=2),
            ResBlockDiscriminator(ndf, ndf, stride=2),
            ResBlockDiscriminator(ndf, ndf, stride=1),
            nn.ReLU(),
        )

        self.disc_net = nn.Sequential(
            SpectralNorm(nn.Conv2d(ndf, 1, 3, 1, 0, bias=False))
        )
        self.aux_net = nn.Sequential(
            SpectralNorm(nn.Conv2d(ndf, opt.num_classes, 3, 1, 0, bias=False))
        )
