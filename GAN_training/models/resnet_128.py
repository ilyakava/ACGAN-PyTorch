...
class BlockGeneratorSpecNorm(nn.Module):
    """
    Add spectral norm like SAGAN suggests
    """

    def __init__(self, in_ch, out_ch, h_ch=None, ksize=3, pad=1,
                 activation=F.relu, upsample=False, num_classes=0):
        super(BlockGeneratorSpecNorm, self).__init__()
        ...
        if self.num_classes > 0:
            ...
            self.b2 = nn.BatchNorm2d(h_ch)
        if self.learnable_sc:
            self.c_sc = utils.spectral_norm(nn.Conv2d(in_ch, out_ch, 1))
...
class GeneratorSpecNorm(nn.Module):
    """Generator generates 128x128."""
    ...
    def __init__(self, num_features=64, dim_z=128, bottom_width=4,
        ...
        num_classes should be the number of real classes
        """
        super(GeneratorSpecNorm, self).__init__()
        self.num_features = num_features
        self.dim_z = dim_z
        self.bottom_width = bottom_width
        self.activation = activation
        ...
        self.l1 = utils.spectral_norm(nn.Linear(dim_z, 16 * num_features * bottom_width ** 2))

        self.block2 = BlockGeneratorSpecNorm(num_features * 16, num_features * 16,
                            activation=activation, upsample=True,
                            num_classes=num_classes)
        self.block3 = BlockGeneratorSpecNorm(num_features * 16, num_features * 8,
                            activation=activation, upsample=True,
                            num_classes=num_classes)
        self.block4 = BlockGeneratorSpecNorm(num_features * 8, num_features * 4,
                            activation=activation, upsample=True,
                            num_classes=num_classes)
        self.block5 = BlockGeneratorSpecNorm(num_features * 4, num_features * 2,
                            activation=activation, upsample=True,
                            num_classes=num_classes)
        self.block6 = BlockGeneratorSpecNorm(num_features * 2, num_features,
                            activation=activation, upsample=True,
                            num_classes=num_classes)
        self.b7 = nn.BatchNorm2d(num_features)
        self.conv7 = utils.spectral_norm(nn.Conv2d(num_features, 3, 1, 1))
...
class Generator64SpecNorm(nn.Module):
    """Generator generates 64x64."""

    def __init__(self, num_features=64, dim_z=128, bottom_width=4,
                 activation=F.relu, num_classes=0, distribution='normal'):
        super(Generator64SpecNorm, self).__init__()
        self.num_features = num_features
        self.dim_z = dim_z
        self.bottom_width = bottom_width
        self.activation = activation
        ...
        self.block2 = BlockGeneratorSpecNorm(num_features * 16, num_features * 8,
                            activation=activation, upsample=True,
                            num_classes=num_classes)
        self.block3 = BlockGeneratorSpecNorm(num_features * 8, num_features * 4,
                            activation=activation, upsample=True,
                            num_classes=num_classes)
        self.block4 = BlockGeneratorSpecNorm(num_features * 4, num_features * 2,
                            activation=activation, upsample=True,
                            num_classes=num_classes)
        self.block5 = BlockGeneratorSpecNorm(num_features * 2, num_features,
                            activation=activation, upsample=True,
                            num_classes=num_classes)
        self.b6 = nn.BatchNorm2d(num_features)
        self.conv6 = utils.spectral_norm(nn.Conv2d(num_features, 3, 1, 1))
